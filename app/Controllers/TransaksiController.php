...
{  
    $data = [
        'items'          => $this->cart->contents(),
        'total'          => $this->cart->total(),
        'voucher_codes'  => array_keys(daftar_voucher())
    ];
...
/**
 * Endpoint AJAX untuk menghitung preview PPN, biaya admin, dan diskon voucher
 * secara real-time di halaman checkout, berdasarkan isi keranjang saat ini
 * dan kode voucher yang diinput pelanggan.
 */
public function hitungCheckout()
{
    $subtotal     = (float) $this->cart->total();
    $voucherCode  = trim((string) $this->request->getGet('voucher_code'));

    $ppn            = hitung_ppn($subtotal);
    $biayaAdmin     = hitung_biaya_admin($subtotal);
    $diskonVoucher  = hitung_diskon_voucher($subtotal, $voucherCode);
    $voucherValid   = is_voucher_valid($voucherCode);

    $subtotalAfterVoucher = $subtotal - $diskonVoucher + $ppn + $biayaAdmin;
...
{ 
    ...
    // Hitung perhitungan tambahan berdasarkan total harga pembelian (belum termasuk ongkir)
    $ppn           = hitung_ppn($subtotal);
    $biayaAdmin    = hitung_biaya_admin($subtotal);
    $diskonVoucher = hitung_diskon_voucher($subtotal, $voucherCode);
    $voucherValid  = is_voucher_valid($voucherCode);
    ...
    $transaction = [
        ...
        'ppn'            => $ppn,
        'biaya_admin'    => $biayaAdmin,
        'voucher_code'   => $voucherValid ? strtoupper($voucherCode) : null,
        'diskon_voucher' => $diskonVoucher,
    ];
